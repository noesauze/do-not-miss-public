...
final class AuthService: NSObject {
    ...
    var currentUserEmail: String? {
        Self.normalizedEmail(currentSession?.accountEmail)
    }
    ...
    func signIn() async throws {
        ...
        let session = OAuthSession(
            ...
            accountEmail: Self.bestEffortAccountEmail(
                explicitEmail: Self.extractEmailFromIDToken(tokenResponse.idToken),
                existingEmail: nil
            )
        )
    ...
    func restoreSessionFromKeychain() {
        let loadedSession: OAuthSession?
        do {
            loadedSession = try keychainHelper.loadCodable(
                service: GoogleOAuthConfig.keychainService,
                account: GoogleOAuthConfig.keychainAccount,
        ...
        guard var restoredSession = loadedSession else {
            currentSession = nil
            return
        }

        let hydratedEmail = Self.bestEffortAccountEmail(
            explicitEmail: Self.extractEmailFromIDToken(restoredSession.idToken),
            existingEmail: restoredSession.accountEmail
        )
        if hydratedEmail != restoredSession.accountEmail {
            restoredSession = OAuthSession(
                ...
                accountEmail: hydratedEmail
            )
            try? persistSession(restoredSession)
        }
    ...
    func getValidAccessToken() async throws -> String {
        ...
        session = OAuthSession(
            ...
            accountEmail: Self.bestEffortAccountEmail(
                explicitEmail: Self.extractEmailFromIDToken(refreshResponse.idToken),
                existingEmail: session.accountEmail
            )
        )
...
private extension AuthService {
    ...
    func makeAuthorizationURL(state: String, codeChallenge: String) -> URL? {
        ...
        components?.queryItems = [
            ...
            URLQueryItem(name: "scope", value: GoogleOAuthConfig.authorizationScope),
    ...
    static func extractEmailFromIDToken(_ idToken: String?) -> String? {
        ...
        return normalizedEmail(object["email"] as? String)
    }

    static func bestEffortAccountEmail(explicitEmail: String?, existingEmail: String?) -> String? {
        normalizedEmail(explicitEmail) ?? normalizedEmail(existingEmail)
    }
